...
def arange_cuda(x, dtype=torch.long):
    return torch.arange(x, dtype=dtype).to(device)
...
def byte_ones_var_cuda(s, requires_grad=False):
    return torch.ones(s, dtype=torch.uint8, requires_grad=requires_grad).to(device)


def ones_var_cuda(s, requires_grad=False, dtype=torch.float32):
    return torch.ones(s, requires_grad=requires_grad, dtype=dtype).to(device)


def int_ones_var_cuda(s, requires_grad=False):
    return torch.ones(s, dtype=torch.long, requires_grad=requires_grad).to(device)


def zeros_like_cuda(x, requires_grad=False, dtype=torch.float32):
    return torch.zeros_like(x, requires_grad=requires_grad, dtype=dtype).to(device)


def byte_zeros_var_cuda(s, requires_grad=False):
    return torch.zeros(s, dtype=torch.uint8, requires_grad=requires_grad).to(device)


def zeros_var_cuda(s, requires_grad=False, dtype=torch.float32):
    return torch.zeros(s, requires_grad=requires_grad, dtype=dtype).to(device)


def int_zeros_var_cuda(s, requires_grad=False):
    return torch.zeros(s, dtype=torch.long, requires_grad=requires_grad).to(device)


def int_fill_var_cuda(s, value, requires_grad=False):
    return torch.zeros(s, dtype=torch.long, requires_grad=requires_grad).to(device) + value


def fill_var_cuda(s, value, dtype=None, requires_grad=False):
    return torch.zeros(s, dtype=dtype, requires_grad=requires_grad).to(device) + value


def byte_var_cuda(x, requires_grad=False):
    tx = torch.ByteTensor(x).to(device)
    if requires_grad:
        tx.requires_grad_()
    return tx
...
def int_var_cuda(x, requires_grad=False):
    tx = torch.IntTensor(x).to(device)
    if requires_grad:
        tx.requires_grad_()
    return tx
...
def long_var_cuda(x, requires_grad=False):
    tx = torch.LongTensor(x).to(device)
    if requires_grad:
        tx.requires_grad_()
    return tx
